...
class APIClienTests: XCTestCase {
  ...
  func test_Login_UsesExpectedURL() {
    ...
    let queryUserName = URLQueryItem(name: "username", value: "dasdöm")
    let queryPassword = URLQueryItem(name: "password", value: "%&34")
    
    XCTAssertTrue((urlComponents?.queryItems?.contains(queryUserName))!)
    XCTAssertTrue((urlComponents?.queryItems?.contains(queryPassword))!)
  }
  ...
  func test_Login_WhenSuccessful_CreatesToken() {
    ...
    waitForExpectations(timeout: 1) { (error) in
      XCTAssertEqual(cachedToken?.id, "1234567890")
    }
  }
